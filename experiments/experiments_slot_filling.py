...
df = pd.read_csv(args.train_data + "p-dialog-dstc2-train.csv", usecols=[1,3,4])
df.columns = ['text','slots','slot_value']
df['category'] = df.slots.values

df.slots = df.slots.fillna('0')
df.slot_value = df.slot_value.fillna('0')

texts = df.text.values
slots = df.slots.values
values = df.slot_value.values

outputs=[]

for i in range(2,3):
  k = 0
  slot = [x.strip() for x in slots[i].split(',')]
  value = [x.strip() for x in values[i].split(',')]
  #print(slot, value)
  out = []
  for word in texts[i]: 
    print(word)  
    if k < len(slot) and word.strip() == value[k]:
      out.append(slot[k])
      k+=1
    else:
      out.append('o')
