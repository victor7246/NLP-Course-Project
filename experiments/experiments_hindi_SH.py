...
#                    help='train data')
parser.add_argument('--train_data', type=str, default='../data/MSH-Comics-Sarcasm/hindi_sarcasm-codemix.txt', required=False,
                    help='train data')
...
parser.add_argument('--model_save_path', type=str, default='models/model_hindi_humour/', required=False,
                    help='seed')
...
df = pd.read_csv(args.train_data, sep='\t',header=None)
df.columns = ['text','category']
kf = StratifiedKFold(n_splits=5, shuffle=True, random_state=args.seed)
for train_index, test_index in kf.split(df.text, df.category):
    break

train_df = df.iloc[train_index]

np.random.shuffle(test_index)
val_index = test_index[0::2]
...
#dummy = np.full(len(train_outputs), 1)
...
'''
new_train_outputs = []
for x in train_outputs:
  if x == 1:
    new_train_outputs.append(0)
  else:
